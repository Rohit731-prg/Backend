...
export const getMessagesByUser = async (req, res) => {
  ...
  try {
    ...
    if (!response) {
      return res
        .status(200)
        .send({ status: false, message: "No messages found" });
    }
...
export const uploadMessage = async (req, res) => {
  ...
  if (!id || !content || !owner) {
    return res
      .status(400)
      .send({ status: false, message: "Id and message and owner is required" });
  }

  if (owner !== "user" && owner !== "admin") {
    return res
      .status(400)
      .send({ status: false, message: "Owner should be user or admin" });
  }
